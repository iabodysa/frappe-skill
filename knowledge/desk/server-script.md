...
frappe/core/doctype/server_script/server_script.py — ServerScript.validate, ServerScript.on_update, ServerScript.on_trash, ServerScript.clear_cache, ServerScript.get_code_fields, ServerScript.scheduled_jobs, ServerScript.sync_scheduled_job_type, ServerScript.check_if_compilable_in_restricted_context, ServerScript.execute_method, ServerScript.execute_doc, ServerScript.execute_scheduled_method, ServerScript.get_permission_query_conditions, execute_api_server_script, enabled
...
NEVER expect a Scheduler Event field change to delete the Scheduled Job Type; on_update calls the single method `sync_scheduled_job_type`, which STOPS the job (`stopped: 1`) when script_type changes away from Scheduler Event, and otherwise updates the SAME job's `method`, `frequency`, `cron_format` and `stopped` (from the script's own `disabled` field) in place whenever event_frequency, cron_format, disabled or script_type changed; nothing in this path deletes a Scheduled Job Type document.
MUST expect deleting the Server Script itself (on_trash) to STOP every one of its Scheduled Job Type rows and clear their `server_script` link, not delete the rows; the schedule entry is left behind, disabled and unlinked.
...
MUST expect a Scheduler Event script to own a Scheduled Job Type whose method is `frappe.scrub("<script name>-<frequency>")`; sync_scheduled_job_type creates it once, on the first sync, and re-syncs the same document afterwards rather than deleting and recreating it.
MUST read a missing Scheduled Job Log as the frequency, not a failure: sync_scheduled_job_type sets `create_log` only ONCE, at creation, from whether event_frequency is neither All nor Cron; changing event_frequency later updates the job's `frequency` field but does not revisit `create_log`.
