...
frappe/www/desk.py — app_name, build_version, desk_theme, layout_direction, include_icons
frappe/www/desk.html — app_name, favicon
...
MUST set `app_name` on Website Settings to rename the desk; www/desk.py reads Website Settings first, System Settings second, and substitutes the literal string "Frappe" when both are empty.
MUST expect the same value to render as the desk page title, because desk.html puts `app_name` straight into the title element.
...
MUST expect a desk asset change to need a new `build_version`, because desk.py stamps the version into the context and the browser caches by it.
