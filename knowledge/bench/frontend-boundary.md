...
frappe/www/desk.html — frappe.boot, _messages
...
MUST call `send_translations` in the server method behind a portal page to put the rows that page needs into the response, because the one generic ajax response handler in `request.js` merges any `__messages` key it finds into `frappe._messages`, whether the response carries a plain reply or a document.
...
desk dictionary: the boot key `__messages`, assigned in `frappe/www/desk.html`
...
run-time top-ups: any `__messages` key on an ajax response, merged by the one generic `.done()` handler in `request.js`, whether the response is a plain reply or carries a document
